...
from utils import slim_ckpt, load_ckpt, seed_everything
...
class NeRFSystem(LightningModule):
    def __init__(self, hparams):
        ...
        with open(path, 'w') as f:
            json.dump(self.net_config, f, indent=4, separators=(", ", ": "), sort_keys=True)
    ...
    def forward(self, batch, split):
        ...
        if self.hparams.optimize_ext:
            ...
            poses[..., :3] = dR @ poses[..., :3]        # Do the rotation for poses
            poses[..., 3] += self.dT[batch['img_idxs']] # Do the translation for poses
    ...
    ### Setup the dataset for training and testing
    ...
    def configure_optimizers(self):
        ...
        ### Exclude the parameters of camera extrinsics
        net_params = []
    ...
    def training_step(self, batch, batch_nb, *args):
        ...
        self.log('lr/xyz_encoder', self.net_opt.param_groups[0]['lr'], True)
        self.log('lr/rgb_net', self.net_opt.param_groups[3]['lr'], True)
    ...
    def on_validation_start(self):
        ...
        if not self.hparams.no_save_test:
            self.val_dir = f'experiments/{self.time}/results/'
...
if __name__ == '__main__':
    ...
    ckpt_cb = ModelCheckpoint(dirpath=f'experiments/{system.time}/ckpts/',
                              filename='{epoch:d}',
    ...
    logger = TensorBoardLogger(save_dir=f"experiments/{system.time}/logs/",
                               name="",
    ...
    if not hparams.val_only: # save slimmed ckpt for the last epoch
        ckpt_ = \
            slim_ckpt(f'experiments/{system.time}/ckpts/epoch={hparams.num_epochs-1}.ckpt',
                      save_poses=hparams.optimize_ext)
        torch.save(ckpt_, f'experiments/{system.time}/ckpts/epoch={hparams.num_epochs-1}_slim.ckpt')
